...
Finds the exact file and line where it occurred
Stores the traceback
Creates a cleaner error message
"""
...
class DocumentPortalException(Exception):
    """Base exception for Document Portal errors."""
    def __init__(self, error_message, error_details: Optional[object] = None):
        ...
        #Resolve exc_info (supports: sys module, Exxeption object or current context)
        exc_type = exc_value = exc_tb = None
        if error_details is None:
            exc_type, exc_value, exc_tb = sys.exc_info()
        else:
            if hasattr(error_details, "exc_info"):  #eg: sys.exc_info()
        ...
        #Walk to the last frame to report the most relevant error location
        last_tb = exc_tb
