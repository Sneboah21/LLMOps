...
# #FastApi can be accessed from any origin, allowing cross-origin requests from any domain. This is useful for development and testing, but in production, you may want to restrict it to specific origins for security reasons.
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],
...
# SESSIONS: Dict[str, List[Dict]] = {}  # Store session data in memory

# #Creating pydantic models inside models.py of model folder
# #Defining routes
# @app.get("/health")
...
# async def upload(files: list[UploadFile] = File(..., description="Select one or more files")) -> UploadResponse:
#     if not files:
